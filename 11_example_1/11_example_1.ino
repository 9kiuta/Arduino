...
void loop() {
  ...
  dist_ema = dist_filtered(dist_ema,dist_raw);
  ...
  float dist_neu = (dist_ema-_TARGET_LOW)/(_TARGET_HIGH - _TARGET_LOW)*(_DUTY_MAX - _DUTY_MIN) + _DUTY_MIN;
  if (dist_ema < _TARGET_LOW) {
    myservo.writeMicroseconds(_DUTY_MIN);
    digitalWrite(PIN_LED, 1);        // LED OFF
  } else if (dist_ema > _TARGET_HIGH) {
...
// ema filter 
float dist_filtered(float dist_ema, float dist_raw)
{
